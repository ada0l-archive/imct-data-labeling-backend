...
from backend.label.schemas import (
    LabelInCreatePydantic,
    LabelInUpdatePydantic,
    LabelPydantic,
    get_label_scheme_by_type,
)
...
def check_data_in_label(project_label, data):
    ...
    try:
        ...
        raise HTTPException(
            detail=e.errors(), status_code=status.HTTP_400_BAD_REQUEST
        )


@router.get("/", response_model=ListPydantic[LabelPydantic])
async def get_labels(
    ...
    labels = await label_rep.get_multi(
        image_id=image_id, pagination=pagination
    )
    return ListPydantic(items=labels)  # type: ignore


@router.post("/", response_model=LabelPydantic)
async def create_label(
    ...
    if not image:
        raise HTTPException(
            detail="Image does not exist", status_code=status.HTTP_409_CONFLICT
        )
...
@router.put("/", response_model=LabelPydantic)
...
@router.get("/{id}", response_model=LabelPydantic)
async def get_label(
    id: int,
    label_rep: LabelRepository = Depends(get_label_rep),
    _=Depends(get_current_user),
